...
public class Board {
	...
	public Board() {
	 ...
	 //TODO: initialise the cells array using ROWS and COLS constants 
		//** +++ create an array of new cell where the rows and cols were constants of GameMain ++++
	...
	public boolean isDraw() {
		...
		//++ Going through each cell to check if there are no empty cell. double for loops below traversing each cell of the rows and columns
		for(int row=0; row < GameMain.ROWS; ++row) {
			for (int col = 0; col < GameMain.COLS; ++col) {
				if(cells[row][col].content == Player.Empty) {
					return false; //++ return false if it is not empty
				}
			}
		}

		return true; //++ return true if there's cell that is empty
	}
	...
	public boolean hasWon(Player thePlayer, int playerRow, int playerCol) {
		...
		if(cells[playerRow][0].content == thePlayer && cells[playerRow][1].content == thePlayer && cells[playerRow][2].content == thePlayer )
		 ...
		 //++ This is to check thePlayer has 3 in the playerCol
		...
		// ++ this is 3-in-the-diagonal in the other direction
